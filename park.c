...
// 在BUS上登記名字，並檢查是否有錯誤
	ret	=dbus_bus_request_name(conn,"park.signal.source",\
...
// 創建信號並檢查錯誤
	msg = dbus_message_new_signal("/test/signal/Object", // path 由/開頭
	"test.signal.Type", // interface 不能有-
	"Test");//name 不能數字開頭不能包含.
	if (NULL == msg)
	{
...
void query(char* param)
{
    ...
    msg = dbus_message_new_method_call("icsim.method.server", // target for the method call
